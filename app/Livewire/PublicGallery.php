    ...
    public function render()
    {
        ...
        if (! empty($this->filterInput)) {
            ...
            if (count($parts) === 2) {
                $type = $parts[0];
                $val = $parts[1];
                $query->whereHas('country', function ($q) use ($type, $val) {
                    if ($type === 'continent') {
                        $q->where('benua', $val);
                    }
                    if ($type === 'subcontinent') {
                        $q->where('subbenua', $val);
                    }
                    if ($type === 'country') {
                        $q->where('nama_inggris', $val);
                    }
                });
            }
        }
        ...
        if ($this->viewMode === 'stamp') {
            ...
            foreach ($stamps as $s) {
                ...
                $cName = $p->country?->nama_inggris ?? 'Unknown';
                ...
                if (! empty($p->contact?->lat) && ! empty($p->contact?->lng)) {
                    // Map Marker Logic
            ...
            foreach ($cards as $p) {
                ...
                if (! empty($p->contact?->lat) && ! empty($p->contact?->lng)) {
                    $mapMarkers[$p->id] = [
                        'id' => $p->id,
                        'postcard_id' => $p->postcard_id,
                        'lat' => (float) $p->contact?->lat,
                        'lng' => (float) $p->contact?->lng,
                        'country' => $cName,
                        'color' => '#e63946',
                    ];
                }
